/// @param read
function scr_sc_transmission_player_xframe() {

	var read = argument[0]

	var network_command = 4
...
#region Read packets
	if read
	{
		...
		if client()
		{
			var X_frame		=  buffer_read(buffer, buffer_u8);
		...
		else
		{
			for (var i=0; i<=ds_list_size(socket_list)-1; i++)
			{
				...
				if _socket2!=socket
				{
					scr_sc_transmission_player_xframe(false, buffer, _socket2)
				}
			}	
		}
	}
...
#region Send packets
	else
	{
		...
		if client()
		{
			buffer_write(Buffer, buffer_u8, x_frame);
